...
export const useErinnerungsEinstellungen = () =>
  useQuery({
    queryKey: ["einstellungen", "erinnerung"] as const,
    queryFn: () => api.get<ErinnerungsEinstellungen>("/einstellungen/erinnerung"),
  });
export const useUpdateErinnerungsEinstellungen = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (data: Partial<ErinnerungsEinstellungen>) =>
      api.patch<ErinnerungsEinstellungen>("/einstellungen/erinnerung", data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["einstellungen", "erinnerung"] }),
  });
};
